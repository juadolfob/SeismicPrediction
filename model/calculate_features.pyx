...
class CalculateFeatures:
    ...
    def __init__(self,
        ...
        cdef all_dates = np.array(df.Datetime, dtype="datetime64[s]")
        cdef all_magnitudes = np.array(df.Magnitude)
        ...
        def _rolling_2_features_apply(_rolling_2_index):
            cdef z_seismic_rate_change = [np.NaN]
            cdef b_seismic_rate_change = [np.NaN]
        ...
        self.features[model.F2_COLUMNS] = np.array(_rolling_2_features_apply(_rolling_2_indexes)).T

        # DELTA TIME FEATURES

        cdef features_last_t_array = np.array(self.features.lastT, dtype='datetime64[s]')
    ...
    def _apply_features_agg(self, df_group):
        cdef old_index = df_group.index.stop - 1
        cdef datetime_array = np.array(df_group.Datetime, dtype='datetime64[s]')
        cdef  magnitudes_array = np.array(df_group.Magnitude)
        cdef t = self.elapsed_time(datetime_array)
        cdef this_window_size = len(df_group.index)
        cdef mean_mag = np.mean(magnitudes_array)
    ...
    def get_trim_features(self):
        ...
        return self.features.drop([0, len(self.features.index) - 1, *_drop_index], axis=0)

    def trim_features(self):
        features_last_t_array = np.array(self.features.lastT, dtype='datetime64[s]')
        _drop_index = self._filter_index_dt(features_last_t_array)
        self.features.drop([0, len(self.features.index) - 1, *_drop_index], axis=0, inplace=True)
    ...
    @staticmethod
    def _gutenberg_richter_curve_fit(np.ndarray[np.float_t] unique, np.ndarray[np.longlong_t] counts):
        return curve_fit(CalculateFeatures.gutenberg_richter_law, unique, counts)[0]

    @staticmethod
    def gutenberg_richter_curve_fit(np.ndarray magnitude, n: int = None):
    ...
    @staticmethod
    def elapsed_time(np.ndarray datetimes_array):
    ...
    @staticmethod
    def rate_square_root_energy(np.ndarray magnitude, int t):
    ...
    @staticmethod
    def mean_time_difference(np.ndarray datetimes):
    ...
    @staticmethod
    def mean_t_deviation(datetimes, int u):
    ...
    def z_seismic_rate_change(self, np.ndarray[np.longlong_t] T):
    ...
    def b_seismic_rate_change(self, np.ndarray[np.longlong_t] T):
    ...
    def _dt_max_magnitude(e_t_dates, d_t, all_dates, all_magnitudes):
        ...
        cdef dt_filter = _positive_dt_filter
        cdef max_mag_d_t = np.array([], dtype="float")
        if d_t < 0:  # negative
    ...
    @staticmethod
    def magnitude_deficit(float max_mag, float a, float b):
        return max_mag - a / b
    ...
    @staticmethod
    def mean_square_deviation(np.ndarray[np.float_t] m_unique,
                              np.ndarray[np.longlong_t] m_count,
                              int n_events,
                              int a,
                              int  b):
    ...
    @staticmethod
    def _b_std(np.ndarray[np.float_t] m_unique, float m_mean, int n_events, float b):
    ...
    @staticmethod
    def p_magnitude(float b, float m):
        return pow(10, -b * m)
